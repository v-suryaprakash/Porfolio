  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (topRowRef.current && bottomRowRef.current) {
        gsap.set(bottomRowRef.current, { x: -150 });
        ...
        gsap.timeline({
          ...
          .to(topRowRef.current, { x: -300, duration: 1 }, 0)
          .to(bottomRowRef.current, { x: 150, duration: 1 }, 0);
      }

      // Horizontal gallery scroll
      if (galleryRef.current && galleryTrackRef.current) {
        const progressIndicator = sectionRef.current?.querySelector<HTMLDivElement>('#gallery-progress') ?? null;
        const getTravelDistance = () => Math.max(0, galleryTrackRef.current!.scrollWidth - window.innerWidth);
        ...
        galleryTimeline.to(galleryTrackRef.current, {
          x: () => -getTravelDistance(),
          duration: 1,
        }, 0);

        if (progressIndicator) {
          galleryTimeline.to(progressIndicator, { width: '100%', duration: 1 }, 0);
        }
      }
    }, sectionRef);
