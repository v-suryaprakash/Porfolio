...
const branchLayout = [
  { targetX: 18.0, targetY: 38.0, anchorX: 42.0, anchorY: 34, cardClass: 'left-[4%] top-[12%] w-[28%]' },
  { targetX: 82.0, targetY: 38.0, anchorX: 58.0, anchorY: 34, cardClass: 'right-[4%] top-[12%] w-[28%]' },
  { targetX: 18.0, targetY: 88.0, anchorX: 42.0, anchorY: 38, cardClass: 'left-[4%] bottom-[12%] w-[28%]' },
  { targetX: 82.0, targetY: 88.0, anchorX: 58.0, anchorY: 38, cardClass: 'right-[4%] bottom-[12%] w-[28%]' },
] as const;
...
// Memoized skill card to prevent re-renders
const SkillCard = memo(function SkillCard({
  ...
  const revealDelay = revealDelays.skills;

  return (
    <motion.div
      key={`${activeFieldId}-${branch.title}`}
      ...
      transition={{ duration: 0.3, delay: revealDelay, ease: [0.2, 1, 0.3, 1] }}
      style={{ willChange: 'transform, opacity' }}
    >
      <TiltCard className="h-full rounded-2xl" maxTilt={8}>
        <div
          className="group relative h-full w-full overflow-hidden rounded-2xl border border-white/12 bg-gradient-to-br from-[#081a2d]/88 via-[#0a2038]/62 to-[#071427]/88 p-4 text-left"
...
// Simplified hologram with fewer animated elements
const HologramCore = memo(function HologramCore({
  field,
  reducedMotion,
}: {
  field: SkillField;
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        {/* Simplified Light Effect - Single layer */}
        <motion.div
          ...
          transition={reducedMotion ? { duration: 0.3, delay: revealDelays.hologram + 0.1 } : { duration: 3, delay: revealDelays.hologram + 0.1, repeat: Infinity, ease: 'easeInOut' }}
        >
          <div className="relative h-[170px] w-[200px] [clip-path:polygon(42%_100%,58%_100%,100%_0%,0%_0%)]">
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(100,240,255,0.3)_0%,transparent_70%)]" />
          </div>
          <div className="absolute bottom-0 h-[200px] w-[240px] [clip-path:polygon(46%_100%,54%_100%,80%_0%,20%_0%)] bg-[radial-gradient(ellipse_at_center,rgba(100,240,255,0.2)_0%,transparent_70%)] blur-[6px]" />
        </motion.div>

        {/* Base Structure - Static, no animations */}
        <div className="pointer-events-none absolute inset-x-0 bottom-[2%] flex flex-col items-center justify-end">
          <div className="relative h-[60px] w-[240px]">
            <div className="absolute bottom-0 left-1/2 h-[45px] w-[240px] -translate-x-1/2 rounded-[50%] border border-cyan-800/40 bg-[#020815] shadow-[0_15px_30px_rgba(0,180,255,0.25)]" />
            <div className="absolute bottom-[10px] left-1/2 h-[45px] w-[230px] -translate-x-1/2 rounded-[50%] border-2 border-cyan-400/30 bg-[#081a33] shadow-[0_0_30px_rgba(50,220,255,0.3)_inset]" />
        ...
        >
          ...
          <motion.h3
            ...
            animate={reducedMotion ? { opacity: 1 } : { opacity: [0.7, 1, 0.85, 1, 0.9] }}
            transition={reducedMotion ? { duration: 0.2 } : { duration: 2, repeat: Infinity, ease: 'easeInOut', delay: revealDelays.label }}
          >
    ...
    >
      <div className="relative z-10 mx-auto flex h-full w-full max-w-7xl flex-col px-6 lg:px-10">
        ...
        <div className="relative mt-5 hidden min-h-0 flex-1 overflow-hidden rounded-[24px] border border-white/10 bg-[#041022]/35 lg:block">
          {/* Simplified SVG - only base lines, no animated pulse */}
          <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            {branchLayout.map((node, index) => {
              const revealDelay = revealDelays.skills;
              const pulseDelay = index * 0.8;

              return (
                <g key={`line-${activeField.id}-${index}`}>
                  <motion.path
                    d={`M 50 42 L ${node.targetX} ${node.targetY}`}
                    fill="none"
                    stroke="rgba(125, 234, 255, 0.25)"
                    strokeWidth="0.15"
                    strokeLinecap="round"
                    initial={{ pathLength: 0, opacity: 0 }}
                    animate={{ pathLength: 1, opacity: 0.7 }}
                    transition={{ duration: 0.5, delay: revealDelay, ease: 'easeOut' }}
                  />
                  {/* Glowing light pulse traveling along the node line */}
                  <motion.line
                    x1="50"
                    y1="42"
                    x2={node.targetX}
                    y2={node.targetY}
                    stroke="rgba(159, 244, 255, 0.9)"
                    strokeWidth="0.5"
                    strokeLinecap="round"
                    initial={{ pathLength: 0, opacity: 0 }}
                    animate={
                    ...
                    transition={{
                      duration: 2.5,
                      delay: revealDelay + 0.5 + pulseDelay,
                      repeat: Infinity,
                      repeatDelay: 1.5,
                      ease: 'linear',
                    }}
          ...
          <div className="absolute inset-x-0 top-[60%] z-20 flex -translate-y-1/2 justify-center px-4 pointer-events-none">
