...
export default function Projects() {
  ...
  const scrollToProjectsAndOpen = useCallback((projectId: string) => {
  ...
  // Use passive scroll listener and throttled transform
  ...
  return (
    <section
      ref={sectionRef}
      id="projects"
      className="relative z-30 w-full min-h-screen overflow-hidden py-16 lg:h-screen lg:py-10 neural-section-layer-strong"
    >
      <motion.div
        aria-hidden="true"
      ...
      <div className="relative z-10 mx-auto flex h-full w-full max-w-7xl flex-col px-6 lg:px-10">
        <motion.div
          initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 30, filter: 'blur(8px)' }}
        ...
        <LayoutGroup id="project-cards">
          <motion.div
            className="mt-6 grid flex-1 grid-cols-1 gap-5 overflow-x-hidden overflow-y-auto pb-4 md:grid-cols-2 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-cyan-300/20 lg:mt-8"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            ...
            {projects.map((project, index) => (
              ...
              >
                {activeProjectId === project.id ? (
                  <div aria-hidden="true" className="h-full rounded-[26px] border border-transparent" />
                ) : (
                  <TiltCard className="h-full rounded-[26px]" maxTilt={6}>
                    <motion.button
                      ...
                      type="button"
                      onClick={() => scrollToProjectsAndOpen(project.id)}
                      data-cursor-label={`Inspect ${project.title}`}
                      className={`group relative h-full w-full overflow-hidden rounded-[26px] border border-white/12 bg-gradient-to-br ${project.gradient} p-5 text-left transition-transform duration-300 hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200/70`}
                    >
                      ...
                      <span className="pointer-events-none absolute right-3 top-3 rounded-full border border-cyan-200/35 bg-cyan-200/15 px-2 py-1 font-mono text-[10px] uppercase tracking-[0.16em] text-cyan-100 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 translate-y-1">
                      ...
                      {project.status === 'In Progress' ? (
                        <div className="relative z-10 flex h-full flex-col">
                          ...
                          <div className="flex flex-1 flex-col items-center justify-center gap-4">
                            <motion.div
                              className="flex h-14 w-14 items-center justify-center rounded-full border border-cyan-200/35 bg-cyan-200/10"
                              animate={
                                reduceMotion
                                  ? undefined
                                  : {
                            ...
                            >
                              <Hammer className="h-7 w-7 text-cyan-100" />
                            </motion.div>

                            <h3 className="font-display text-3xl text-cyan-100">Crafting...</h3>
                          </div>
                        </div>
                      ) : (
                        <div className="relative z-10 flex h-full flex-col justify-between gap-3">
                          <div>
                            ...
                            <h3 className={`mt-2 font-display text-2xl text-white ${project.accent}`}>
                              {project.title}
                            </h3>
                            <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-white/72">{project.summary}</p>
                          </div>
                          ...
                          <div className="space-y-3">
                            <div className="flex flex-wrap gap-2">
                              {project.stack.slice(0, 3).map((tech) => (
                                <span
                                  key={tech}
                                  className="rounded-full border border-white/18 bg-white/8 px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.14em] text-white/72"
                                >
                                  {tech}
                                </span>
                              ))}
